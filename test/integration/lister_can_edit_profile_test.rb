...
class ListerCanEditProfileTest < ActionDispatch::IntegrationTest
  test "lister can edit profile" do
    ...
    #assert page.has_content?(lister.file_upload_file_name)
    ...
    #page.attach_file("file_upload", Rails.root + fixture_image_path)
    ...
    assert page.has_content?(lister.zipcode)
    assert page.has_content?(lister.bio)
    #refute page.has_css?("img[src*='#{original_lister.file_upload_file_name}']")
